...
knitr::include_graphics("photos/HerbVar-phylogeny-2020-04-02.png")
```
...
ggplot(focal.sp.map.data, aes( x= long, y = lat, group = group)) +
  ...
  scale_fill_gradient(low='grey90', high=viridis(2)[1], breaks=0:max(focal.sp.map.data$Plantago.count)) +
  guides(fill = guide_legend(reverse = T)) +
  labs(fill = 'No. surveys'
...
```{r, echo=FALSE, warning=FALSE, out.width = "800px", fig.asp=0.5, fig.align='center', fig.cap="Sampling areas with at least one completed survey (yellow) and planned sampling areas (purple). We’re continually adding new planned sites."}
