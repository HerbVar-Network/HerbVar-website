...
As of November 2019, the Herbivory Variability Network has completed surveys on > 125 plant species from > 50 plant families. These figures and tables update as new data are entered into our database, so check back often! 
...
register_google('AIzaSyAn3tOz8Y60I7Yh8Wk7RuD1xk_oUW6wJU8')
...
plannedSites = geocode(plannedLocations)
...
ggplot() + 
	...
	theme(axis.title=element_blank(), axis.text=element_blank(),
		axis.ticks=element_blank()) + 
	geom_point(aes(x=plannedSites$lon, y=plannedSites$lat), fill=viridis(2,)[1], size=2.5,
		shape=21, alpha=0.5) +
